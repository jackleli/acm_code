...
1. 约数个数： $d(n)=(k_1+1)(k_2+1)...(k_m+1)$
2. 约数和： $\sum_{d|n}d=(p_1^0+p_1^1+...+p_1^{k_1})(p_2^0+p_2^1+...+p_2^{k_2})...(p_m^0+p_m^1+...+p_m^{k_m})$
...
#### BFS染色法判定二分图 && 求出每个二分连通块的两侧点集
```c++
vector<int> color(n + 1, -1);
vector<array<int, 2>> comps;
bool ok = true;
for (int i = 1; i <= n; i++) {
...
对于一个无向简单图 $G=(V,E)$，它的补图记作 $\overline G$。
...
也就是说，原图中有边的点对，在补图中没有边；原图中没有边的点对，在补图中有边。

#### 团与独立集互补
- 团：点集 $S$ 在原图 $G$ 中是团，当且仅当 $S$ 中任意两个不同点之间都有边：

$$
\forall u,v\in S,u\ne v,\quad (u,v)\in E(G)
$$

- 独立集：点集 $S$ 在图 $G$ 中是独立集，当且仅当 $S$ 中任意两个不同点之间都没有边：

$$
\forall u,v\in S,u\ne v,\quad (u,v)\notin E(G)
$$

由于补图中边的存在性和原图完全相反，所以有一个常用结论：

$$
S\text{ 是 }G\text{ 中的团}
\iff
S\text{ 是 }\overline G\text{ 中的独立集}
$$

因此，如果题目要求把所有点划分成 $S_1,S_2$，并且要求 $S_1,S_2$ 在原图中都是团，那么等价于：

$$
S_1,S_2\text{ 在 }\overline G\text{ 中都是独立集}
$$

#### 团划分与二分图

如果在补图 $\overline G$ 中， $S_1,S_2$ 都是独立集，并且：

$$
S_1\cap S_2=\varnothing,\quad S_1\cup S_2=V
$$

那么补图中的每条边都必须连接 $S_1$ 和 $S_2$ 中的点，不可能出现在同一个集合内部。
